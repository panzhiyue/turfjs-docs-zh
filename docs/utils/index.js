...
const POINT_COORDINATES2 = [119.73426818847659, 28.06890058517456];
...
const MULTILINESTRING_COORDINATES = [
  ...
  LINESTRING_COORDINATES2,
];

const POLYGON_COORDINATES2 = [
  [
...
const MULTIPOLYGON_COORDINATES = [POLYGON_COORDINATES, POLYGON_COORDINATES2];

const POINT_GEOMETRY = new Point(POINT_COORDINATES);
const POINT_GEOMETRY2 = new Point(POINT_COORDINATES2);
...
const LINESTRING_GEOMETRY2 = new LineString(LINESTRING_COORDINATES2);
...
const POLYGON_GEOMETRY2 = new Polygon(POLYGON_COORDINATES2);
...
const POINT_FEATURE2 = new Feature({
  geometry: POINT_GEOMETRY2,
});
...
const LINESTRING_FEATURE2 = new Feature({
  geometry: LINESTRING_GEOMETRY2,
});
...
const POLYGON_FEATURE2 = new Feature({
  geometry: POLYGON_GEOMETRY2,
});
...
const FEATURECOLLECTION_POINT = [POINT_FEATURE, POINT_FEATURE2];
const FEATURECOLLECTION_LINESTRING = [LINESTRING_FEATURE, LINESTRING_FEATURE2];
const FEATURECOLLECTION_POLYGON = [POLYGON_FEATURE, POLYGON_FEATURE2];
...
export const getTestOL = (type) => {
  switch (type) {
    ...
    case "FeatureCollection<Point>": {
      return FEATURECOLLECTION_POINT;
    }
    case "FeatureCollection<LineString>": {
      return FEATURECOLLECTION_LINESTRING;
    }
    case "FeatureCollection<Polygon>": {
      return FEATURECOLLECTION_POLYGON;
    }
  }
};
...
export const getTestFeatures = (type) => {
  let testOL = getTestOL(type);
  if (type.indexOf("FeatureCollection") > -1) {
    return testOL;
  } else if (type.indexOf("Feature") > -1) {
    return [testOL];
  } else {
